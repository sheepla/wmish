...
- `NAMESPACE <Namespace>`: Move around namespaces hieralchy like `cd` command
- `CLASSES`: Show a list of classes on the current namespace like `ls` command
...
- `SELECT <Properties...> FROM <ClassName> WHERE <Conditions>...`: Query to WMI in WQL (WMI Query Language)
- `FORMAT CSV|TABLE|JSON`: Set output format
- `CALL <MethodName> <<PropertyName1=Value1> <PropertyName=Value2> ...> WITH <Query|ClassName>`
- `MOF <ClassName>`: Get MOF (Managed Object Format) of specified class name
